...
### 2. Pull Calendar via tools

Use the available MCP tools to fetch the calendar and the events details.
...
### 6. Gather Context from Memory, Knowledge Bases, and Emails

1. Search in related KBs, recent modified files in Onedive, memories, and emails for relevant information.
2. Check recent meeting summaries or plans for relevant prep context based on long memory
3. Use this to generate actionable prep bullets per meeting
...
- **MS365 MCP tools** must be available for calendar and email access (Microsoft 365 / Outlook)
